...
void regen_linear(double** x, double* reduced_buffer, int ratio_x, int ratio_y,  int me, int* xs, int* xe, int* ys, int* ye,  int xcell, int ycell )
{
   /* Index variables */
   int i, j;
     ...
     for(j=0;j<ycell;j++)
        for(i=0;i<xcell;i++)
        { // if(x[xs[me]+ratio_x*i][ys[me]+j*ratio_y]-reduced_buffer[index]>2)
     //     printf("%d,%d,%d,%f,%f\n",me,xs[me]+ratio_x*i,ys[me]+j*ratio_y,x[ys[me]+j*ratio_y][xs[me]+ratio_x*i],reduced_buffer[index]);
          x[ys[me]+j][xs[me]+i]=reduced_buffer[i]-x[ys[me]+j][xs[me]+i];
