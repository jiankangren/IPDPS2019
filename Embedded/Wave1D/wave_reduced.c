...
void collect ( int id, int p, int n_global, int n_local, int nsteps, 
		double dt, double u_local[], MPI_Comm comm, int nproc_reduced);
	 ...
	 */
{
	...
	if ( id == 0 ) 
	{
		timestamp ( );
		printf ( "\n" );
		printf ( "Reduced: MPI_WAVE:\n" );
		printf ( "Reduced:  Using %d processes.\n", p );
		printf ( "Reduced:  Using a total of %d points.\n", n_global );
		printf ( "Reduced:  Using %d time steps of size %g.\n", nsteps, dt );
		printf ( "Reduced:  Computing final solution at time %g\n", dt * nsteps );
	}
	 ...
	 */
	collect ( id, p, n_global, n_local, nsteps, dt, u1_local , comm, nproc_reduced );
	/*
	...
	if ( id == 0 )
	{
		printf ( "Reduced: Elapsed wallclock time was %g seconds\n", wtime );
	...
	if ( id == 0 )
	{
		printf ( "Reduced: Normal end of execution.\n" );
		timestamp ( );
	}
	 ...
	 */
{
	...
	for ( i = 1; i <= nsteps; i++ )
	{
		...
		/*if(id==0&&i<1000){
		  int step_full;
		  MPI_Send(&i,1,MPI_INT,32,0,MPI_COMM_WORLD);
		  MPI_Recv(&step_full,1,MPI_INT,32,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
		  }
		 */
		if ( i == 1 )
		{
	 ...
	 */
{
	...
	if ( id == 0 )
	{
		...
		//int luo;
		FILE * file;
		file=fopen("reduced_data.dat","w");
		for ( i_global = 0; i_global < n_global; i_global++)
		{
			//                        printf("%f\n",u_global[i_global]);
			fprintf(file,"%f",u_global[i_global]);
		}
		fclose(file);


		int nproc_full=64;
		int ratio=(n_global-1)/nproc_full;
		for (i = 0; i < nproc_full; i++ )
		{
			MPI_Send( &u_global[ratio*i], ratio+1, MPI_DOUBLE, i+nproc_reduced, 1,MPI_COMM_WORLD);
			//  MPI_Send( &ratio, 1, MPI_INT, i+nproc_reduced, 1,MPI_COMM_WORLD);
