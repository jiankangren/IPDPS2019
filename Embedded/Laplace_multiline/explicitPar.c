...
{
<<<<<<< HEAD:Embedded/Laplace_multiline/explicitPar.c
	/* Arrays */
	double **x0;
	double **x;
	double *xfinal;
	...
	regen_linear(x,x0, size_x/size_x_reduced, size_y/size_y_reduced,me, xs, xe, ys, ye, xcell, ycell);
...
int world_rank;
   ...
   if(me==0){
    readParam(iconf,conf);
 readParam(iconf_reduced,conf_reduced);
}
   MPI_Bcast(iconf,5,MPI_INT,0,comm);
   MPI_Bcast(conf,2,MPI_DOUBLE,0,comm);
MPI_Bcast(iconf_reduced,5,MPI_INT,0,comm);
...
int *x_index,*y_index;
   ...
   return 0;
>>>>>>> c7c9e0ee8edba03204298fe0ab7e55a72e63d718:Heat2D/explicitPar.c
}
