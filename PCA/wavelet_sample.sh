...
#List="protein-Ligand Free-energy lysozyme umbrella Virtual-Sites"
List="Heat3d	Laplace Wave	umbrella	Virtual-Sites	astro	fish	sedov-pres	yf17_temp"
#List="Wave"
#List="Heat3d Laplace Free-energy lysozyme protein-Ligand umbrella Virtual-Sites astro blast2-pres bump_dense dpot eddy_velx_f4 fish sedov-pres yf17_pres yf17_temp"
set -- $List
err=32
err1=4
for i
...
#./transfer $i.dat.1 $i.dat.1.b
    python  wavelet.py Inputdata\_sampling/$i\_sample.dat
    echo "Wavelet  Finish!"
   # base_size='0'
  ...
  # Inputdata/transfer  uk.txt low_base
  # size='0'
...
/home/luo/Tao/LossyCompressStudy/SZ/example/testdouble_compress sz1.config delta  $size
   ...
   zfp_size=$(( $(( $(stat -c%s base.zfp)/3 )) + $(stat -c%s delta.zfp) ))
   sz_size=$(( $(( $(stat -c%s base.sz)/3 )) + $(stat -c%s delta.sz) ))
